...
---
<!-- trackio-cell
{"type": "code", "id": "cell_eb88ef7cce44", "created_at": "2026-07-29T11:58:46+00:00", "title": "verify five source constructions", "command": ["python3", "repro/src/verify.py"], "exit_code": 0, "duration_s": 0.278}
-->
````bash
...
---
<!-- trackio-cell
{"type": "code", "id": "cell_c274f6d7c52d", "created_at": "2026-07-29T11:58:47+00:00", "title": "strict publication gate", "command": ["python3", "repro/src/publication_gate.py"], "exit_code": 0, "duration_s": 0.068}
-->
````bash
...
#!/usr/bin/env python3
"""Fail-closed local publication gate for 29sn1uqWn3."""
from __future__ import annotations
import json
from pathlib import Path
...
assert len(claims) == 5 and verdict["all_claims_passed"]
assert all(item.get("passed") and item.get("source") and item.get("mechanism") and item.get("negative_control") for item in claims.values())
assert (root / "RESULTS.md").is_file() and (root / "docs" / "SOURCE_AUDIT.md").is_file()
gate = {"paper": "29sn1uqWn3", "arxiv": "2603.03612", "publication_eligible": True,
        "tests_passed": True, "publication_gate_passed": True, "claim_count": 5,
        ...
        "scope": "five source-anchored theory constructions; finite executable checks plus public TeX proof anchors"}
(root / "outputs" / "publication_gate.json").write_text(json.dumps(gate, indent=2, sort_keys=True) + "\n")
(root / "GATE_READY.md").write_text("FULL_GATE_READY: 29sn1uqWn3\n")
